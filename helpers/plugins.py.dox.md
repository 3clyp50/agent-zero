...
- `get_plugin_update_state() -> tuple[str, dict[str, PluginUpdateInfo]]`, `save_plugin_update_state(...)`, and `clear_plugin_update(...)`: persist plugin remote-commit status used by the Custom plugin list; clearing advisory status is best-effort after a successful update.
...
- Custom plugin list items expose the last daily remote-commit result as
  `update_available`; list rendering never fetches remotes.
